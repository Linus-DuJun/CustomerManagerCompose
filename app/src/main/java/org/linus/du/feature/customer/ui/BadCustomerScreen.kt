...
@Composable
private fun BadCustomerContentView(
    viewModel: BadCustomerViewModel,
    onCheckCustomerDetailInfo: (Customer) -> Unit,
    onShowBottomSheet: (Customer) -> Unit
) {
    val badCustomers = viewModel.badCustomers.collectAsLazyPagingItems()
    LazyColumn(
        modifier = Modifier
            .bodyWidth()
            .padding(start = 16.dp)
    ) {
        items(items = badCustomers) { customer ->
            BadCustomerItemView(
                customer = customer!!,
                onCheckCustomerDetailInfo = onCheckCustomerDetailInfo,
                onShowBottomSheet = onShowBottomSheet
        ...
        badCustomers.apply {
            when {
                ...
                loadState.refresh is LoadState.Error -> {
                    val e = badCustomers.loadState.refresh as LoadState.Error
                    // TODO add error item and retry
                }
                loadState.append is LoadState.Error -> {
                    val e = badCustomers.loadState.append as LoadState.Error
                    item {
                        ErrorItem(
...
@Composable
private fun BadCustomerItemView(
    customer: Customer,
    onCheckCustomerDetailInfo: (Customer) -> Unit,
    onShowBottomSheet: (Customer) -> Unit
) {
